/**
 * System Prompt 构建器
 * 
 * 管理和构建 AI 的系统提示词
 */

/**
 * 基础系统提示词
 */
const BASE_PROMPT = `You are a helpful assistant. 你是一个友好的 AI 助手。`

/**
 * 富媒体格式说明
 */
const MEDIA_FORMAT_PROMPT = `
当需要展示特定类型的信息时，请使用以下格式：

1. 天气信息 - 使用 weather 代码块：
\`\`\`weather
{"city": "城市名", "temp": 温度数字, "condition": "天气状况", "humidity": 湿度数字}
\`\`\`

2. 数据图表 - 使用 chart 代码块：
\`\`\`chart
{"type": "bar或line", "title": "图表标题", "labels": ["标签1", "标签2"], "values": [数值1, 数值2]}
\`\`\`

图表限制：
- 仅支持 bar（柱状图）和 line（折线图）两种类型
- 不支持饼图(pie)、散点图(scatter)、雷达图(radar)等其他类型
- 如果用户要求不支持的图表类型，请用文字说明"目前仅支持柱状图和折线图"，并建议使用支持的类型

3. 图片生成规则（重要！）：
- 当用户要求生成/画/创作图片时，立即调用 generate_image 工具，不要先说话
- 直接调用工具，不要说"正在生成"之类的话
- 绝对禁止自己编造图片 URL 或输出任何图片格式
- 禁止输出 \`\`\`image 格式、Markdown 图片语法、占位符链接
- 工具返回后图片会自动展示，你只需简单确认即可
- 如果没有 generate_image 工具，告知用户"图片生成功能暂不可用"

4. 搜索规则（重要！）：
- 当需要查询实时信息、新闻、最新数据时，立即调用 web_search 工具，不要先说话
- 直接调用工具，不要说"正在搜索"之类的话
- 绝对禁止假装搜索或编造搜索结果
- 工具返回后，根据搜索结果整理回答
- 如果没有 web_search 工具，告知用户"搜索功能暂不可用"

5. 工具调用原则：
- 需要工具时，立即调用，不要先输出文字
- 一次请求中可以同时调用多个工具
- 工具调用失败时再用文字解释原因

注意：
- 只在用户明确询问天气或需要数据可视化时才使用 weather/chart 格式
- 必须通过工具调用来执行搜索和图片生成，不能自己编造结果
- 工具调用优先于文字输出，先做事再说话
`

/**
 * 构建完整的系统提示词
 */
export function buildSystemPrompt(): string {
  return BASE_PROMPT + MEDIA_FORMAT_PROMPT
}

/**
 * 构建消息上下文
 */
export function buildContextMessages(
  historyMessages: Array<{ role: string; content: string }>,
  currentUserMessage: string
): Array<{ role: string; content: string }> {
  return [
    {
      role: 'system',
      content: buildSystemPrompt(),
    },
    // 历史消息
    ...historyMessages.map((msg) => ({
      role: msg.role,
      content: msg.content,
    })),
    // 当前用户消息
    {
      role: 'user',
      content: currentUserMessage,
    },
  ]
}

/**
 * 处理附件，将文件内容添加到消息中
 */
export function appendAttachments(
  content: string,
  attachments?: Array<{ name: string; content: string }>
): string {
  if (!attachments || attachments.length === 0) {
    return content
  }

  const fileContents = attachments
    .map((file) => `\n\n---\n**附件: ${file.name}**\n\`\`\`\n${file.content}\n\`\`\``)
    .join('\n')

  return content + fileContents
}
